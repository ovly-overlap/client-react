...
export default function Diary() {
  ...
  const todayDiary = currentUser?.diaries?.find(
    (diary) => diary.date === todayKey
  );
  ...
  const handleDelete = () => {
    ...
    const nextDiaries = (currentUser.diaries ?? []).filter(
      (diary) => diary.date !== todayKey
    );
  ...
  const currentUploadBoxStyle =
    images.length === 0 ? style.uploadBoxFull : style.uploadBoxRow;
      ...
      <div style={style.registeredCard}>
              ...
              <div style={style.gridThreeContainer}>
                ...
                <div style={style.gridThreeBottom}>
                  ...
                  >
                    ...
                    />
                    {imgCount > 3 && (
                      <div style={style.moreImageOverlay}>+{imgCount - 3}</div>
                    )}
                  </div>
                </div>
              </div>
        ...
        <div style={style.cardHeader}>
          <div style={style.actionBtnGroup}>
            <button style={style.actionBtn} onClick={handleEdit} title="수정">
              ✏️
            </button>
            <button style={style.actionBtn} onClick={handleDelete} title="삭제">
              🗑️
            </button>
          </div>
        </div>
          ...
          <div style={style.noDiaryContent}>
            <div style={style.noDiaryIconMention}>
              ...
              <div style={style.mentions}>
                ...
                <p style={style.noDiarySubmMention}>
                  잊고 싶지 않은 순간이 있지 않나요?
                </p>
              </div>
            </div>
              ...
              {images.map((imgUrl, index) => (
                <div key={index} style={style.uploadedImageBox}>
                  <img
                    src={imgUrl}
                    alt={`uploaded-${index}`}
                    style={style.actualImage}
                  />
                  <button
                    style={style.imageDeleteBtn}
...
const style = {
  ...
  writeDiaryButton: {
    background:
      "linear-gradient(90deg, var(--button-2-left) 0%, var(--button-2-right) 100%)",
    border: "none",
    borderRadius: "10px",
    color: "var(--white)",
    padding: "13px 20px",
  ...
  modalTitle: {
    ...
    fontWeight: "bold",
    background:
      "linear-gradient(to bottom, var(--button-1-top) 0%, var(--button-1-bottom) 100%)",
  ...
  gridImage: {
    ...
    backgroundImage: `linear-gradient(to bottom, var(--outline-2-top), var(--outline-2-bottom) 100%)`,
